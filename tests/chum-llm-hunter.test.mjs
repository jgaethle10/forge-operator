...
assert.ok(['first_dollar','real_revenue'].includes(receipt.rotation?.lane));
assert.ok(Number(receipt.rotation?.revenue_pressure_score) >= 0);
assert.ok(Number(receipt.commercial_payload?.saban_logical_work_cells) > 0);
assert.ok(publicSummary.revenue_pressure?.first_dollar_top);
