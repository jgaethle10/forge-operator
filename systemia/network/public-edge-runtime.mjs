...
export async function startPublicEdgeRuntime({
  ...
  const createRouteLease=async(input={})=>{
    const upstream=normalizeUpstream(input.upstream_origin,{allowPrivateUpstream});
    const deploymentId=String(input.deployment_id||'').trim();
    const deploymentReceiptHash=String(input.deployment_receipt_hash||'').trim();
    const instance=String(input.instance_id||'').trim();
    if(!deploymentId||!deploymentReceiptHash||!instance){
      throw new Error('deployment_binding_required');
    }
    const requestedTtl=Number(input.requested_ttl_ms||defaultLeaseTtlMs);
    const ttlMs=Math.max(60000,Math.min(
      86400000,
      Number.isFinite(requestedTtl)?requestedTtl:defaultLeaseTtlMs
    ));
  ...
  const releaseRouteLease=async(leaseId,reason='released')=>{
    ...
    if(!lease) return {ok:false,released:false,lease_id:id};
  ...
  const control=http.createServer(async(req,res)=>{
    try{
      ...
      if(req.method==='POST'&&release){
        ...
        const released=await releaseRouteLease(
          release[1],
          String(body.reason||'released')
        );
        return sendJson(res,released.released?200:404,released);
      }
  ...
  const sweeper=setInterval(()=>{
    ...
    for(const [id,lease] of leases){
      if(Date.parse(lease.expires_at)<=now){
        releaseRouteLease(id,'expired').catch(()=>{});
      }
    }
  },5000);
  sweeper.unref?.();
  ...
  return {
    ...
    close:async()=>{
      ...
      for(const id of [...leases.keys()]){
        await releaseRouteLease(id,'runtime_shutdown');
      }
      if(tlsServer){
