...
export function invokeForensiScopeGatewayTool({
  ...
  if (name === 'forensiscope_verify_analysis') {
    ...
    if (!evidenceRef || !provenanceRef) {
      throw new Error(
        'ForensiScope verification requires evidence_ref and provenance_ref.'
      );
    }
    if (!accessToken) {
    ...
    const access = verifyAndCheckAccess({
      accessToken,
      evidenceRef,
  ...
  if (name === 'forensiscope_compare_evidence') {
    ...
    return {
      ...
      access: [accessSummary(accessA), accessSummary(accessB)],
      audit,
      result,
      authority: authorityBoundary()
    };
  ...
  const accessToken = String(args.access_token || '');
  if (!evidenceRef) {
    throw new Error('ForensiScope gateway requires evidence_ref.');
  }
  if (!accessToken) {
    throw new Error('ForensiScope gateway requires access_token.');
  }
