...
function publicOffer(offer) {
  return {
    public_id: String(offer.public_id || offer.id || ''),
...
try {
  response = await fetch(LIVE_CATALOG_URL, {
    headers: {
      ...
      'user-agent': 'Evercraft-CHUM/0.4 (+public-catalog-sync)'
    },
...
const rows = Array.isArray(live?.capabilities)
  ? live.capabilities
  : Array.isArray(live?.offers)
    ? live.offers
    : null;

if (live?.ok !== true || !rows) {
  throw new Error('Live catalog response is not a valid Evercraft public catalog.');
}

const offers = rows
  .map(publicOffer)
  .filter((x) => x.public_id && x.name && x.commercial_state !== 'superseded')
  .sort((a, b) => a.public_id.localeCompare(b.public_id));

const next = {
  schema: 'evercraft.machine-catalog.snapshot.v2',
  provider: 'Evercraft LLC',
  purpose:
    'Public-safe snapshot of the canonical Systemia machine-commerce catalog for AI/search/agent discovery. This file grants no private access and creates no payment authority.',
  source_url: LIVE_CATALOG_URL,
  source_service: String(live.service || ''),
  ...
  safety: {
    ...
    ...(live.rules && typeof live.rules === 'object' ? live.rules : {})
  },
  offers
};
