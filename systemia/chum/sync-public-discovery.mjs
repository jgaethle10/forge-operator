...
async function main() {
  ...
  try {
    response = await fetch(LIVE_CATALOG_URL, {
      headers: {
        ...
        'user-agent': 'Evercraft-CHUM/0.2 (+public-catalog-sync)'
      },
  ...
  const next = {
    ...
    purpose:
      'Public-safe snapshot of the canonical Evercraft machine-commerce catalog for AI/search/agent discovery. This file grants no private access and creates no payment authority.',
  ...
  if (
    current &&
    JSON.stringify(semanticSnapshot(current)) === JSON.stringify(semanticSnapshot(next))
  ) {
    console.log(
      JSON.stringify({
        changed: false,
