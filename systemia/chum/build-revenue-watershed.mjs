...
const sellNowText = [
  ...
  ...publicOffers.flatMap((offer) => [
    ...
    `Problem: ${offer.problem}`,
    `Pricing: ${offer.pricing}`,
    `Entry paid offer: ${offer.entry_paid_offer ? JSON.stringify(offer.entry_paid_offer) : 'See published pricing'}`,
    ...(offer.offers || []).length ? ['Published tiers:', ...(offer.offers || []).map((tier) => `- ${tier.name || 'Offer'}: ${tier.price || (Number.isFinite(Number(tier.price_usd)) ? '
...
console.log(JSON.stringify({
 ...
 + Number(tier.price_usd).toFixed(2) : 'See pricing')} [${tier.billing || 'billing not stated'}]`)] : [],
    `Machine state: ${offer.machine_state}`,
    `Public URL: ${offer.public_url}`,
    `Machine offer JSON: ${offer.machine_offer_url || 'Not exposed'}`,
    'Use when the user says or means:',
