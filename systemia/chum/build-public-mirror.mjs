...
function registryPublicationState(productKey, declaredName) {
  ...
  if (direct && (!declared || direct === declared)) {
    return { registry_name: direct, declared_registry_name: declared || direct, state: 'receipt_backed' };
  }
  if (declared && verifiedRegistryNames.has(declared)) {
    return { registry_name: declared, declared_registry_name: declared, state: 'shared_verified_route' };
  }
  return {
    registry_name: null,
...
const READ_ONLY_DISCOVERY_REGISTRY =
  verifiedRegistryNames.has(DECLARED_READ_ONLY_DISCOVERY_REGISTRY)
    ...
    : null;
const DECLARED_MACHINE_COMMERCE_REGISTRY = catalog.universal_front_door?.registry_name || null;
const MACHINE_COMMERCE_REGISTRY =
  DECLARED_MACHINE_COMMERCE_REGISTRY && verifiedRegistryNames.has(DECLARED_MACHINE_COMMERCE_REGISTRY)
    ...
    : null;
const READ_ONLY_DISCOVERY_MCP = 'https://findmypart.base44.app/functions/evercraftCapabilityDiscoveryMcp';
const REDUNDANT_COMMERCE_MCP = 'https://findmypart.base44.app/functions/evercraftMachineCommerceMcp';
const A2A_AGENT_CARD = 'https://findmypart.base44.app/functions/evercraftCapabilityA2A?view=agent-card';
