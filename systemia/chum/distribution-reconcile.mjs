...
try {
  response = await fetch(OPERATOR_URL, {
    headers: {
      ...
      'user-agent': 'Evercraft-CHUM/0.4 (+distribution-reconcile)'
    },
...
if (strict && (protocolFailure || founderLeak)) {
  throw new Error(
    `CHUM distribution strict failure: protocols_ok=${!protocolFailure} founder_attention_required=${founderLeak}`
  );
}
