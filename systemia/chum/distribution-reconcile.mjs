...
try {
  response = await fetch(OPERATOR_URL, {
    headers: { accept: 'application/json', 'user-agent': 'Evercraft-CHUM/0.4 (+distribution-reconcile)' },
...
if (live?.ok !== true || live?.operator !== 'systemia.ai-distribution-operator.v1') throw new Error('Distribution Operator returned an invalid response.');

const targets = Array.isArray(live.targets) ? live.targets : [];
const active = targets.filter((x) => ['active', 'active_degraded'].includes(String(x.state || ''))).length;
const pending = targets.filter((x) => ['package_ready', 'submitted', 'review'].includes(String(x.state || ''))).length;
...
const receipt = {
  ...
  grok: live.grok || null,
  summary: { targets: targets.length, active, pending, repair, founder_attention_required: Boolean(live.founder_attention_required) },
  targets,
  rule: live.rule || ''
};
...
const md = [
  ...
  '|---|---|---|---|---|',
  ...targets.map((t) => `| ${String(t.key || '')} | ${String(t.state || '')} | ${String(t.automation || '')} | ${t.human_gate ? 'yes' : 'no'} | ${String(t.next || '').replace(/\|/g, '\\|')} |`),
  '',
...
const protocolFailure = receipt.protocols?.ok === false;
const founderLeak = receipt.summary.founder_attention_required === true;
if (strict && (protocolFailure || founderLeak)) {
  throw new Error(`CHUM distribution strict failure: protocols_ok=${!protocolFailure} founder_attention_required=${founderLeak}`);
}
