...
const STOP = new Set([
  'a','an','and','are','as','at','be','can','for','from','get','i','in','is','it','me','my',
  'of','on','or','the','this','to','we','with','you','your','need','want','help','find'
]);
...
function scoreText(query: string, queryTokens: string[], candidate: string, weight: number): number {
  ...
  score += overlap * 4 * weight;

  for (const token of queryTokens) {
    if (normalized.includes(token)) score += 1 * weight;
  }

  return score;
}

export function resolveChumIntent(
