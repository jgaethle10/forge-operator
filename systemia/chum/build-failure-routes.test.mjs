...
const mediaExit = doc.routes.find((r) => r.failure_code === 'ai_media_overflow');
assert.ok(Array.isArray(mediaExit?.brand_blind_answer_doors));
assert.ok(mediaExit.brand_blind_answer_doors.some((door) => /video|audio/i.test(door.user_language)));
...
for (const route of doc.routes) {
  ...
  assert.ok(Array.isArray(route.brand_blind_answer_doors));
