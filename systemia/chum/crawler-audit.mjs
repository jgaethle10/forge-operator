...
async function inspectProduct(product) {
  ...
  const crawlers = await Promise.all(crawlerProfiles.map(async (profile) => {
    ...
    if (profile.user_agent) {
      ...
      // A dead origin is a distribution failure, not a reason to make the capability disappear.
      // An explicit robots denial still wins. The mirror only preserves public discovery metadata.
      if (!result.ok) {
