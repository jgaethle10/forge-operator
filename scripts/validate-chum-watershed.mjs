...
for (const bot of ['OAI-SearchBot','GPTBot','ClaudeBot','Claude-SearchBot','Googlebot','Google-Extended','PerplexityBot','Applebot']) {
...
const card = json('public/.well-known/agent-card.json');
if (!card.name || !card.url || !Array.isArray(card.skills)) fail('agent card missing identity, endpoint or skills');
else pass('agent card is machine-readable');
...
else pass('OpenAPI exposes wired public invocation paths');
