...
function loadPublicDiscoveryCatalog(): any {
  ...
  const knownNames = new Set(offers.map((offer: any) => normalizedName(offer?.name)).filter(Boolean));
  ...
  for (const product of directory?.products || []) {
    ...
    if (knownNames.has(normalizedName(name)) || knownUrls.has(canonical.replace(/\/$/, ''))) continue;

    const intents = Array.isArray(product?.intents) ? product.intents.map(String).filter(Boolean) : [];
    offers.push({
      public_id: `product-${String(product?.product_key || normalizedName(name).replace(/\s+/g, '-'))}-discovery`,
      name,
  ...
  if (!q) {
    res.status(400).json({
      ...
      example: req.method === 'POST'
        ? { problem: 'I need a discontinued tractor part' }
        : '/api/discover?q=I%20need%20a%20discontinued%20tractor%20part',
