...
function escapeChumHtml(value: unknown): string {
...
app.get('/chum/buy/:publicId', rateLimit(240, 60 * 60 * 1000), (req: Request, res: Response) => {
  ...
  try {
    ...
    const tierMarkup = tiers.length
      ? tiers.map((tier: any) => {
          const name = escapeChumHtml(tier?.name || tier?.offer_key || 'Paid option');
          const price = escapeChumHtml(tier?.price || (Number.isFinite(Number(tier?.price_usd)) ? '
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.json({
...
setupServer();
 + Number(tier.price_usd).toLocaleString('en-US') : 'See published pricing'));
          const billing = tier?.billing ? ' · ' + escapeChumHtml(String(tier.billing).replace(/_/g, ' ')) : '';
          return '<li><strong>' + name + '</strong><span>' + price + billing + '</span></li>';
        }).join('')
      : '<li><strong>Published pricing</strong><span>' + escapeChumHtml(offer.pricing || 'See offer review') + '</span></li>';
    ...
    const detailUrl = typeof offer.public_url === 'string' && /^https:\/\//i.test(offer.public_url)
      ? offer.public_url
      : null;
    const canonicalOrigin = requestOrigin(req);
    const canonicalUrl = canonicalOrigin
      ? canonicalOrigin + '/chum/buy/' + encodeURIComponent(offer.public_id)
    ...
    res.type('html').send([
      ...
      '<p>' + escapeChumHtml(offer.confirmation || 'You choose whether to continue. No payment obligation is created by viewing this page.') + '</p>',
      '<a class="cta" href="' + escapeChumHtml(continuePath) + '" aria-label="Continue to purchase review for ' + escapeChumHtml(offer.name) + '">Continue to purchase review</a>',
      detailUrl ? '<p><a class="secondary" href="' + escapeChumHtml(detailUrl) + '">Open capability details</a></p>' : '',
