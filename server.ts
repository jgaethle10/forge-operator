  ...
  for (const relative of ['sitemap.xml', 'chum/sitemap.xml']) {
    try {
      ...
      for (const match of xml.matchAll(/<loc>([^<]+)<\/loc>/g)) {
        const value = String(match[1] || '').trim().replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
...
app.get('/robots.txt', (req: Request, res: Response) => {
  ...
  const publicApi = ['Allow: /api/health','Allow: /api/capabilities','Allow: /api/commercial','Allow: /api/discover','Allow: /api/revenue-watershed'];
  const lines = [
    ...
    '',
    'User-agent: OAI-SearchBot','User-agent: GPTBot','User-agent: ChatGPT-User',
    'User-agent: Claude-SearchBot','User-agent: Claude-User','User-agent: ClaudeBot',
    'User-agent: Googlebot','User-agent: Google-Extended','User-agent: bingbot','User-agent: PerplexityBot',
    'Allow: /','Allow: /llms.txt','Allow: /llms-full.txt','Allow: /.well-known/','Allow: /chum/',
    ...publicApi,'Disallow: /api/','',
    'User-agent: *','Allow: /','Allow: /llms.txt','Allow: /llms-full.txt','Allow: /.well-known/','Allow: /chum/',
    ...publicApi,'Disallow: /api/','',
...
app.get('/sitemap.xml', (req: Request, res: Response) => {
  ...
  const body = ['<?xml version="1.0" encoding="UTF-8"?>','<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">',...urls.map((url) => '  <url><loc>' + xmlEscape(url) + '</loc></url>'),'</urlset>',''].join('\n');
